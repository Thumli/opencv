...
private:
    template<typename T>
    static CV_CONSTEXPR std::true_type check_param(typename std::is_same<typename std::decay<decltype(std::declval<T>().operator()(std::declval<Args>()...))>::type, Ret>::type*);

    template<typename> static CV_CONSTEXPR std::false_type check_param(...);

    typedef decltype(check_param<C>(0)) type;
